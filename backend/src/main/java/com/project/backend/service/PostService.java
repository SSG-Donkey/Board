...
//        }
    ...
    //게시글 나눔중으로 되돌리기 변경
    public int sharePost(int post_no,int user_no)  {
        ...
        int give_point=postMapper.givePoint(find_no,find_point); //채택유저 포인트 업데이
        int cut_point=postMapper.cutPoint(user_no,find_point); //작성자 포인트 원래대로 복구
